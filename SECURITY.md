...
| Recuperação de senha por e-mail | ✅ | "Esqueci minha senha" envia um link de **uso único, válido por 30 min** (`/api/auth/recuperar-senha` → `/api/auth/redefinir-senha`). No banco fica só o hash SHA-256 do segredo do link; um novo pedido invalida o anterior; redefinir **derruba todas as sessões abertas** da conta. A resposta é idêntica exista o e-mail ou não e o e-mail sai *depois* da resposta (`after()`), para não revelar quem está cadastrado; máx. 3 pedidos por e-mail e 10 por IP em 15 min. O link é montado com `APP_URL`, nunca com o cabeçalho `Host` (evita link envenenado). Senha fraca é recusada **antes** de gastar o link |
