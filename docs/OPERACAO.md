...
| `AUTH_SECRET` | **obrigatória** (≥ 32 caracteres) | Assina as sessões, cifra as chaves do app autenticador e gera os HMAC dos códigos. Sem ela o app recusa todo acesso (503) |
...
7. Confirme com o teste de fumaça contra o ambiente: `SMOKE_BASE_URL=https://… SMOKE_ADMIN_EMAIL=… SMOKE_ADMIN_PASSWORD=… pnpm smoke` (cria e desativa um usuário de teste; use num ambiente de homologação).
...
- **Perdeu o acesso de administrador** (senha esquecida sem SMTP, conta desativada): o sistema impede rebaixar/desativar o último, mas se acontecer, defina `SEED_ADMIN_EMAIL` e `SEED_ADMIN_PASSWORD` e rode `pnpm admin:ensure` — recria ou restabelece o administrador (ativo, com a nova senha, **sem verificação em 2 etapas** — e-mail e app são desligados, com os códigos de recuperação —, derrubando as sessões dele) sem apagar nenhum dado. Ligue o 2FA de novo depois de entrar.
- **"A trilha foi adulterada"**: não apague nada. Anote o id do registro apontado, preserve um `pg_dump` do estado atual e compare com o backup mais recente. Se a causa foi troca de `AUTH_SECRET`/`AUDIT_SEAL_SECRET`, o aviso aparece em todos os registros antigos, não em um só.
...
- O limite de tentativas de login e de códigos fica **em memória** de cada instância. Com mais de uma instância (ou reinício), o contador zera/divide; para produção com várias instâncias, troque por um armazenamento compartilhado (ex.: Redis) — está descrito no topo de `lib/server/auth/rate-limit.ts`.
